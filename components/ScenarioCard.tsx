...
const ScenarioCard: React.FC<ScenarioCardProps> = ({ scenario, currentTeam, onOptionSelected }) => {
  ...
  const getIcon = () => {
    switch (scenario.iconType) {
      case 'database': return <Database size={48} className="w-10 h-10 md:w-16 md:h-16 text-emerald-400" />;
      case 'mobile': return <Smartphone size={48} className="w-10 h-10 md:w-16 md:h-16 text-purple-400" />;
      case 'strategy': return <Zap size={48} className="w-10 h-10 md:w-16 md:h-16 text-yellow-400" />;
      case 'system': return <Globe size={48} className="w-10 h-10 md:w-16 md:h-16 text-blue-400" />;
      default: return <Briefcase size={48} className="w-10 h-10 md:w-16 md:h-16 text-gray-400" />;
    }
  };

  return (
    <div className="w-full max-w-6xl mx-auto mt-2 md:mt-6 animate-pop pb-8 px-2 md:px-0">
      {/* Simulation Header */}
      <div className="flex justify-between items-center mb-2 md:mb-4 px-1 md:px-4">
        <span className="bg-slate-800 text-slate-400 px-2 md:px-4 py-1 rounded-md text-[10px] md:text-sm font-mono border border-slate-700 truncate max-w-[50%]">
          ID: {scenario.id.toUpperCase()}
        </span>
        <div className="flex items-center gap-1.5 md:gap-2 text-red-400 animate-pulse">
          <AlertTriangle size={14} className="md:w-5 md:h-5" />
          <span className="font-bold tracking-widest uppercase text-[10px] md:text-base">Action Required</span>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 md:gap-8 items-start">
          ...
          <div className="flex-grow">
            <div className="flex items-center gap-3 mb-3 md:hidden">
                 ...
                 <h2 className="text-xl font-black text-white brand-font leading-tight">
                    {scenario.title}
                 </h2>
            </div>
      ...
      {/* Decision Interface */}
      <div className="bg-slate-800 rounded-b-xl md:rounded-b-3xl p-4 md:p-8 border-t border-slate-700 shadow-xl">
      ...
      <div className="mt-4 md:mt-6 text-center">
         <span className="text-slate-400 text-xs md:text-base">Current Decision Authority:</span>
         <span className="ml-2 font-black text-lg md:text-2xl text-white tracking-wide">{currentTeam.name}</span>
      </div>
    </div>
  );
};
